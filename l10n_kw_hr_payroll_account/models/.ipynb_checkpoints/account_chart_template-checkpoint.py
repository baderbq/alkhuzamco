...
class AccountChartTemplate(models.Model):
    ...
    def configure_kuw_payroll_account(self, companies):
        ...
        accounts_codes = [
            '965001',  # Payables
            '965002',  # End of Service Provision
            '965003',  # Basic Salary
            '965004',  # Housing Allowance
            '965005',  # Transportation Allowance
        ...
        for company in companies:
            ...
            # ================================================ #
            #          Kuwait Employee Payroll Structure       #
            # ================================================ #
            ...
            self.env['hr.salary.rule'].search(salary_rule_domain_basic, limit=1).write({'account_debit': accounts['965003'].id})
            # self.env.ref('l10n_ae_hr_payroll.uae_basic_salary_rule').write({"account_debit": accounts['400003'].id})
            self.env.ref('l10n_kw_hr_payroll.kuwait_housing_allowance_salary_rule').write(
                {"account_debit": accounts['965004'].id})
            self.env.ref('l10n_kw_hr_payroll.uae_transportation_allowance_salary_rule').write(
                {"account_debit": accounts['965005'].id})
            self.env.ref('l10n_kw_hr_payroll.uae_other_allowances_salary_rule').write(
                {"account_debit": accounts['965012'].id})
            # self.env.ref('l10n_ae_hr_payroll.uae_net_salary_rule').write({"account_credit": accounts['201002'].id})
            self.env.ref('l10n_kw_hr_payroll.uae_end_of_service_salary_rule').write(
                {"account_debit": accounts['965002'].id})
            self.env.ref('l10n_kw_hr_payroll.uae_end_of_service_provision_salary_rule').write(
                {"account_debit": accounts['965008'].id, "account_credit": accounts['965002'].id})
