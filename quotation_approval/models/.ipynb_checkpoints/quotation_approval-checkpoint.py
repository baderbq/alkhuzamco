...
class SaleOrder(models.Model):
    ...
    def action_approve(self):
        if self.company_id.so_order_approval and self.company_id.approver_ids:
            if self._uid in  self.company_id.approver_ids.ids:
                msg="""<h2 style="overflow-y: hidden;"><a title="Click to check this Quotation" style="background-color: #01666b; padding: 4px 8px 4px 8px; text-decoration: none; color: #fff; border-radius: 5px; font-size:13px;" href="#" data-oe-model="sale.order" data-oe-id="%s">%s</a> </h2> """ % (str(self.id),str(self.name))

                message = "Hello,<br/>"+ self.env.user.name+" has <strong style=\"font-size:19px\"><u>Approved</u></strong> your request for Quotation N°"+str(msg)
    ...
    def request_approve(self):
        ...
        chanel_obj = self.env['mail.channel']
        if self._uid in  self.company_id.approver_ids.ids:
            raise UserError("you can't make request for approvals\n because you are approver.")
        if self.company_id.so_order_approval and self.company_id.approver_ids:
